...
def _decode_block_str(block_str):
    ...
    if block_type == 'ir':
        ...
        if 'ui' in options: block_args['use_ir'] = True
        if 'rs' in options: block_args['residual'] = True
        if 'xy' in options: block_args['constant_Dxy'] = True

        if 'hoh' in options: block_args['init_h0_h'] = True
        if 'mof' in options: block_args['no_f'] = True
        if 'udot' in options: block_args['use_dot'] = True
        if 'ucds' in options: block_args['use_cDs'] = True

        if 'dx' in options: block_args['dx'] = int(options['dx'])
        if 'dy' in options: block_args['dy'] = int(options['dy'])
        if 'dt' in options: block_args['dt'] = float(options['dt'])
        if 'cDx' in options: block_args['cDx'] = float(options['cDx'])
        if 'cDy' in options: block_args['cDy'] = float(options['cDy'])
...
class EfficientNetBuilder:
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            _log_info_if('  GlobalFeatureBlock_Diffusion {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
